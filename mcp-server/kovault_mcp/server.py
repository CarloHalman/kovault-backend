...
def _cols(table: str, refresh: bool = False) -> dict[str, dict]:
    ...
    # A6, hard lock: vector columns are filtered out on the way OUT, so NO tool built on `_cols`
    # can emit one. One embedded chunk serialises its halfvec(4000) to ~13k tokens and `rows`
    # defaults to 50, so a single call could have returned ~650k tokens. That was patched in `rows`
    # alone; enforcing it here means a newly added tool cannot reopen the hole by forgetting the
    # same filter. A model never needs to read an embedding.
    # Filtered here rather than when the cache is filled, so it holds however the cache was
    # populated — a lock that only guards one write path is not a lock.
    # Raw access stays available through the debug-gated `sql` tool, which writes its own SQL and
    # never consults this reflection — as do _vector_hits and the embed worker, both of which name
    # `embedding` literally. Matched on udt_name because data_type says 'USER-DEFINED' for every
    # enum too, so `tasks.status` would otherwise look like a vector.
    return {c: m for c, m in _COLS_CACHE[table].items() if m.get("udt") not in _VECTOR_UDTS}
...
def _check_columns(table: str, fields: dict, kind: str | None = None) -> list[str]:
    ...
    for col, val in fields.items():
        ...
        if meta is None:
            # The deny list is checked off its own static set, not off the reflection: `_cols` hides
            # vector columns from every tool (A6), so `embedding` arrives here as meta=None and the
            # loop below would never reach it. A denied column must still be refused by name rather
            # than passed over as merely unknown.
            if col in _DENY_COLS:
                out.append(f"{key} is set by the server and cannot be written")
            continue                          # other unknown columns are reported by the parser
...
def _precise_lookup(tables, filters, count, limit, offset, columns=None,
                    groups_include=None, groups_exclude=None) -> str:
    ...
    groups_include/exclude are applied HERE as well as in ranked mode. They used not to be: lookup()
    dispatched to precise mode before the group sets were ever resolved, so a group-scoped precise
    query silently returned the whole table — a confident wrong count, which is worse than an error
    in an audit path. This is the mode audits actually use."""
    ...
    # A header belongs to a group through its PAGE, exactly as ranked mode scopes it (group_entity).
    gcol = "page_id" if table == "headers" else "id"
    if groups_include:
        inc, err = _group_entity_sets(groups_include)
        if err:
            return err
        # An include that resolves to nothing means no row qualifies. Returning everything instead
        # is how this bug read from the outside, so an empty set filters to empty, never to all.
        clauses.append(f"{gcol} = ANY(%s)")
        params.append(list(inc))
    if groups_exclude:
        exc, err = _group_entity_sets(groups_exclude)
        if err:
            return err
        if exc:
            clauses.append(f"NOT ({gcol} = ANY(%s))")
...
def lookup(
    ...
    for table in tables:
        ...
        for cid in set(vhits) | set(khits) | set(thits):
            ...
            # `groups_include or []` was asked for, so an empty resolved set means "nothing
            # qualifies" — not "skip the filter". Keyed off the REQUEST, not off the set being
            # non-empty, or a group that resolves to no members returns the whole index instead.
            if groups_include and group_entity not in inc_groups:
                continue
...
def rows(table: str, where: list[dict] | None = None, limit: int = 50) -> str:
    ...
    for cond in where or []:
        ...
        clauses.append(clause)
        params.append(param)
    # A6: never SELECT * here — name the columns, which `_cols` has already stripped of vectors for
    # every tool at once (see the hard lock there). Listing them explicitly still matters: SELECT *
    # would go straight to the table and bypass that filter entirely.
...
# Trigram name similarity. Tuned against a real vault: the genuine duplicate pairs there measured
# 0.64 and 0.48, so 0.4 clears both with margin. The noise this would otherwise catch is numbered
# benchmark series scoring 0.80-0.81 — removed by the numbered-series exclusion below, not by this
# number, since raising the threshold to dodge that noise would also cut the genuine 0.48 pair.
...
def _dup_groups_by_name(cap: int = _DUP_GROUP_PAIR_CAP) -> list[dict]:
    ...
    stripping trailing digits/separators reduces 'run-batch-4' and 'run-batch-6' to the same stem,
    and an identical stem means series, not duplicate. Finds what content-overlap structurally
    cannot: genuine duplicate groups often share zero members. Sizes are along for context (a merge
